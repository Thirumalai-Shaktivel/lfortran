...
namespace LCompilers {
    ...
    llvm::AllocaInst* LLVMUtils::CreateAlloca(llvm::IRBuilder<> &builder, llvm::Type* type,
            llvm::Value* size, std::string Name, bool is_llvm_ptr) {
        llvm::AllocaInst *alloca;
        if (type != character_type)
            LCOMPILERS_ASSERT(!type->isPointerTy())
    ...
    llvm::Value *LLVMUtils::CreateLoad(llvm::Type *t, llvm::Value *x) {
        return LLVM::CreateLoad2(*builder, t, x);
    }
    ...
    llvm::Value* LLVMUtils::create_ptr_gep(llvm::Value* ptr, int idx) {
        ...
        llvm::ConstantInt::get(context, llvm::APInt(32, idx))};
        llvm::Type *type = nullptr;
        if (ptr_type.find(ptr) != ptr_type.end()) {
            type = ptr_type[ptr];
        }
        LCOMPILERS_ASSERT(type);
                                             ...
                                             std::string runtime_func_name, llvm::Module& module)
    {
        ...
        llvm::AllocaInst *pleft_arg = CreateAlloca(character_type);
        LLVM::CreateStore(*builder, left_arg, pleft_arg);
        llvm::AllocaInst *pright_arg = CreateAlloca(character_type);
    ...
    llvm::Value* LLVMUtils::is_equal_by_value(llvm::Value* left, llvm::Value* right,
        ...
        switch( asr_type->type ) {
            ...
            case ASR::ttypeType::Character: {
                str_cmp_itr = CreateAlloca(llvm::Type::getInt32Ty(context));
    ...
    void LLVMDict::rehash(llvm::Value* dict, llvm::Module* module,
        ...
        idx_ptr = llvm_utils->CreateAlloca(llvm::Type::getInt32Ty(context), nullptr);
        LLVM::CreateStore(*builder, llvm::ConstantInt::get(llvm::Type::getInt32Ty(context),
