...
namespace LCompilers {
    ...
    namespace LLVMArrUtils {
        ...
        get_pointer_to_dimension_descriptor_array(llvm::Value* arr, bool load) {
            ...
            return llvm_utils->CreateLoad(dim_des, dim_des_arr_ptr);
        }
    ...
    void print_util(llvm::Type* v, std::string endline="\n") {
        ...
        void SimpleCMODescriptor::fill_malloc_array_details(
            ...
            llvm::Value *arr_ = llvm_utils->CreateLoad(arr);
            llvm::Value* offset_val = llvm_utils->create_gep(arr_, 1);
            builder->CreateStore(llvm::ConstantInt::get(context, llvm::APInt(32, 0)),
                                    offset_val);
            llvm::Value* dim_des_val = llvm_utils->CreateLoad(dim_des->getPointerTo(), llvm_utils->create_gep(arr_, 2));
            llvm::Value* prod = llvm::ConstantInt::get(context, llvm::APInt(32, 1));
            for( int r = 0; r < n_dims; r++ ) {
        ...
        llvm::Value* SimpleCMODescriptor::get_lower_bound(llvm::Value* dims, bool load) {
            ...
            llvm::Type *i32 = llvm::Type::getInt32Ty(context);
            return llvm_utils->CreateLoad(i32, lb);
        }

        llvm::Value* SimpleCMODescriptor::get_upper_bound(llvm::Value* dims) {
            llvm::Type *i32 = llvm::Type::getInt32Ty(context);
            llvm::Value* lb = llvm_utils->CreateLoad(i32, llvm_utils->create_gep2(dim_des, dims, 1));
            llvm::Value* dim_size = llvm_utils->CreateLoad(i32, llvm_utils->create_gep2(dim_des, dims, 2));
        ...
        llvm::Value* SimpleCMODescriptor::get_stride(llvm::Value* dims, bool load) {
            llvm::Value* stride = llvm_utils->create_gep(dims, 0);
            if( !load ) {
                return stride;
            }
...
// print_util(dim_des_arr_ptr);
            ...
            for( int r = 0; r < n_args; r++ ) {
                ...
                llvm::Value* lval = llvm_utils->CreateLoad(i32, llvm_utils->create_gep2(dim_des, dim_des_ptr, 1));
                ...
                llvm::Value* stride = llvm_utils->CreateLoad(i32, llvm_utils->create_gep2(dim_des, dim_des_ptr, 0));
                idx = builder->CreateAdd(idx, builder->CreateMul(stride, curr_llvm_idx));
            }
            llvm::Value* offset_val = llvm_utils->CreateLoad(i32, llvm_utils->create_gep(arr, 1));
        ...
        llvm::Value* SimpleCMODescriptor::get_single_element(llvm::Type *type, llvm::Value* array,
            ...
            if( data_only || is_fixed_size ) {
                ...
                if( polymorphic ) {
                    ...
                    tmp = llvm_utils->create_ptr_gep2(type, llvm_utils->CreateLoad(type->getPointerTo(), full_array), idx);
// print_util(tmp, ":this\n");
                }
            }
            return tmp;
        ...
        llvm::Value* SimpleCMODescriptor::get_is_allocated_flag(llvm::Value* array,
            ...
            return builder->CreateICmpNE(
                builder->CreatePtrToInt(get_pointer_to_data(array),
        ...
        llvm::Value* SimpleCMODescriptor::get_array_size(llvm::Value* array, llvm::Value* dim, int kind, int dim_kind) {
            ...
            llvm::Value* llvm_size = llvm_utils->CreateAlloca(llvm_utils->getIntType(kind), nullptr);
            ...
            llvm::Value* r = llvm_utils->CreateAlloca(llvm_utils->getIntType(4), nullptr);
