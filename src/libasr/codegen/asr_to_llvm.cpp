...
public:
    ...
    llvm::Value* CreateLoad(llvm::Value *x) {
        ...
        return LLVM::CreateLoad2(*builder, type, x);
    }
    ...
    llvm::Value* CreateGEP(llvm::Value *x, std::vector<llvm::Value *> &idx) {
        ...
        LCOMPILERS_ASSERT(type);
        return LLVM::CreateGEP2(*builder, type, x, idx);
    }
    ...
    */
        ...
        for( size_t i = 0; i < x.n_args; i++ ) {
            ...
            if( n_dims == 0 ) {
                ...
                if (ASRUtils::is_character(*curr_arg_m_a_type)) {
                    ...
                    builder->CreateCall(fn, args);
                    builder->CreateMemSet(CreateLoad(x_arr),
                ...
                llvm_utils->create_if_else(
                    builder->CreateICmpEQ(
                        builder->CreatePtrToInt(x_arr, llvm::Type::getInt32Ty(context)),
                ...
                if( ASR::is_a<ASR::StructType_t>(*ASRUtils::extract_type(ASRUtils::expr_type(tmp_expr)))) {
                    allocate_array_members_of_struct_arrays(CreateLoad(x_arr),
                        ASRUtils::expr_type(tmp_expr));
                }
            }
        }
    ...
    inline void call_lfortran_free(llvm::Function* fn, llvm::Type* llvm_data_type) {
        llvm::Value* arr = arr_descr->get_pointer_to_data(tmp);
        ...
        std::vector<llvm::Value*> args = {llvm_utils->CreateLoad(character_type, arg_arr)};
        builder->CreateCall(fn, args);
        ...
        for( size_t i = 0; i < x.n_vars; i++ ) {
            ...
            if( ASR::is_a<ASR::Var_t>(*tmp_expr) ) {
                ...
                if (ASR::is_a<ASR::StructType_t>(*caller_type)) {
                    ...
                    dt = CreateLoad(llvm_utils->create_gep(dt, 1));
                } else {
                    LCOMPILERS_ASSERT(false);
                }
    ...
    void visit_ArrayItem(const ASR::ArrayItem_t& x) {
        ...
        if (ASRUtils::is_character(*x.m_type) && n_dims == 0) {
            ...
            if (((is_bindc_array && !ASRUtils::is_fixed_size_array(m_dims, n_dims)) &&
    ...
    void add_wasm_start_function() {
        ...
        args.push_back(CreateAlloca(character_type));
    ...
    */
    ...
    void allocate_array_members_of_struct_arrays(llvm::Value* ptr, ASR::ttype_t* v_m_type) {
        ...
        llvm::Value* array_size = llvm_utils->CreateAlloca(*builder,
        ...
        llvm::Value* llvmi = llvm_utils->CreateAlloca(*builder, llvm::Type::getInt32Ty(context), nullptr, "i");
        LLVM::CreateStore(*builder,
            llvm::ConstantInt::get(llvm::Type::getInt32Ty(context), llvm::APInt(32, 0)), llvmi);
        create_loop(nullptr, [=]() {
                llvm::Value* llvmi_loaded = CreateLoad(llvmi);
                llvm::Value* array_size_loaded = CreateLoad(array_size);
            ...
            [=]() {
                ...
                switch (phy_type) {
                    case ASR::array_physical_typeType::FixedSizeArray: {
                        ptr_i = llvm_utils->create_gep(ptr, CreateLoad(llvmi));
                        break;
                    }
                    case ASR::array_physical_typeType::DescriptorArray: {
                        ptr_i = llvm_utils->create_ptr_gep(
                            CreateLoad(arr_descr->get_pointer_to_data(ptr)),
                            CreateLoad(llvmi));
                        break;
                    }
                    case ASR::array_physical_typeType::PointerToDataArray: {
                        ptr_i = llvm_utils->create_ptr_gep(ptr, CreateLoad(llvmi));
                        break;
                    }
                    default: {
                        LCOMPILERS_ASSERT(false);
                ...
                LLVM::CreateStore(*builder,
                    builder->CreateAdd(CreateLoad(llvmi),
            ...
            */
            ...
            if( !compiler_options.stack_arrays && array_size ) {
                ...
                if (v->m_storage == ASR::storage_typeType::Save) {
                    ...
                    ptr = llvm_utils->CreateAlloca(*builder, type_, array_size, v->m_name,
                        LLVM::is_llvm_pointer(*v->m_type));
// print_util(type_);
// print_util(ptr);
                }
            ...
            if( init_expr != nullptr &&
                ...
                if( ASRUtils::is_array(v->m_type) &&
                    ...
                    builder->CreateStore(CreateLoad(init_value), target_var);
            ...
            if (x.m_tgt) {
                ...
                if( ASRUtils::is_array(ASRUtils::expr_type(x.m_tgt)) ) {
                    ...
                    if( tgt_ptype != ASR::array_physical_typeType::DescriptorArray ) {
                        ptr = CreateLoad(arr_descr->get_pointer_to_data(ptr));
                    }
                }
    ...
    void handle_array_section_association_to_pointer(const ASR::Associate_t& x) {
        ...
        if( ASR::is_a<ASR::StructInstanceMember_t>(*array_section->m_v) &&
            ASRUtils::extract_physical_type(value_array_type) !=
                ASR::array_physical_typeType::FixedSizeArray ) {
            value_desc = CreateLoad(value_desc);
        }
        ptr_loads = 0;
    ...
    void visit_Assignment(const ASR::Assignment_t &x) {
                ...
                if( is_value_list_to_array ) {
                    ...
                    if( ASRUtils::extract_physical_type(asr_target_type) ==
                        ...
                        array_data = LLVM::CreateLoad(*builder,
                            arr_descr->get_pointer_to_data(CreateLoad(target)));
        ...
        if( ASRUtils::is_array(target_type) &&
            ...
            if( is_value_fixed_sized_array && is_target_fixed_sized_array ) {
                ...
                value = CreateLoad(arr_descr->get_pointer_to_data(value));
                target = llvm_utils->create_gep(target, 0);
                ASR::dimension_t* asr_dims = nullptr;
                size_t asr_n_dims = ASRUtils::extract_dimensions_from_ttype(target_type, asr_dims);
                int64_t size = ASRUtils::get_fixed_size_of_array(asr_dims, asr_n_dims);
                ...
                if( is_target_data_only_array ) {
                    ...
                    target_data = CreateLoad(arr_descr->get_pointer_to_data(target));
                }
                if( is_value_data_only_array ) {
                    value_data = value;
                    ...
                    value_data = CreateLoad(arr_descr->get_pointer_to_data(value));
                }
    ...
    void visit_ArrayPhysicalCastUtil(llvm::Value* arg, ASR::expr_t* m_arg,
        ...
        if( m_new == ASR::array_physical_typeType::PointerToDataArray &&
            ...
            tmp = CreateLoad(arr_descr->get_pointer_to_data(arg));
            ...
            builder->CreateStore(llvm_utils->create_ptr_gep(
                CreateLoad(arr_descr->get_pointer_to_data(tmp)),
    ...
    void visit_IfExp(const ASR::IfExp_t &x) {
        ...
        llvm::Value* ifexp_res = llvm_utils->CreateAlloca(*builder, _type, nullptr, "");
        llvm_utils->create_if_else(cond, [&]() {
...
// tmp->print(llvm::errs()<<":this\n\n");
    ...
    void visit_BitCast(const ASR::BitCast_t& x) {
        ...
        llvm::Value* source_ptr = llvm_utils->CreateAlloca(*builder, source_type, nullptr, "bitcast_source");
        ...
        tmp = CreateLoad(builder->CreateBitCast(source_ptr, target_llvm_type));
    }
    ...
    void visit_Cast(const ASR::Cast_t &x) {
        ...
        switch (x.m_kind) {
            ...
            case (ASR::cast_kindType::ListToArray) : {
                ...
                this->visit_expr(*x.m_arg);
                ptr_loads = ptr_loads_copy;
                tmp = CreateLoad(list_api->get_pointer_to_list_data(tmp));
                break;
            }
    ...
    void visit_FileRead(const ASR::FileRead_t &x) {
        ...
        if (x.m_iostat) {
            ...
            iostat = llvm_utils->CreateAlloca(*builder, llvm::Type::getInt32Ty(context));
        }
        ...
        if (x.m_size) {
            ...
            read_size = llvm_utils->CreateAlloca(*builder, llvm::Type::getInt32Ty(context));
        }
    ...
    void visit_FileInquire(const ASR::FileInquire_t &x) {
        ...
        if (x.m_exist) {
            ...
            exist_val = llvm_utils->CreateAlloca(*builder, llvm::Type::getInt1Ty(context));
        }
        ...
        if (x.m_opened) {
            ...
            opened_val = llvm_utils->CreateAlloca(*builder, llvm::Type::getInt1Ty(context));
        }
    ...
    void visit_SubroutineCall(const ASR::SubroutineCall_t &x) {
        ...
        if( x.m_dt && ASR::is_a<ASR::StructInstanceMember_t>(*x.m_dt) &&
            ...
            ptr_loads = ptr_loads_copy;
            llvm::Value* callee = CreateLoad(tmp);
